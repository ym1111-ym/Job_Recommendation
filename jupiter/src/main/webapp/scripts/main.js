(function() {
	...
	function register() {
		...
		//regular expression的检查 正则表达式
		if (username.match(/^[a-z0-9_]+$/) === null) {
			showRegisterResult('Invalid username');
			return;
		}
		/*md5是引入的library  用来做密码的加密 在index.html里面引入的
		 * 这个library的地址在哪里？ browser？其实是在window下
		 * 可以在网页inspect console处输入window.md5 看到返回的是一个方法
		 */
		...
		/*
		 * ajax 传入的是一个method，一个url， 一个req，一个成功的回调函数，一个error的函数
		 */
		ajax('POST', url, req,
		// successful callback
		function(res) {
